...
const FreeStanding = () => {
    ...
    return (
        <Box sx= {{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', overflowX: 'scroll', '&::-webkit-scrollbar': { display: 'none' }, marginTop: '10px' }}>
            <Ctrl.Checkbox name= { `construction_inspection.draw.free_standing.fs` } register= { register(`construction_inspection.draw.free_standing.fs`, { onChange: () => setIsFS(!isFS) }) }
                size= "large" checked= { defaultVal !== undefined ? defaultVal.draw.free_standing.fs > 0 ? true : isFS : isFS } />
            ...
            <Box sx= {{ margin: '0 20px', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                ...
                <Box sx= {{ border: 'solid 1px #dcdde1', borderRadius: '5px', padding: '5px 15px' }}>
                    <Ctrl.TextField name= { `construction_inspection.draw.free_standing.w` } register= { register(`construction_inspection.draw.free_standing.w`) }
            ...
            <Box sx= {{ margin: '0 20px', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                ...
                <Box sx= {{ border: 'solid 1px #dcdde1', borderRadius: '5px', padding: '5px 15px' }}>
                    <Ctrl.TextField name= { `construction_inspection.draw.free_standing.h` } register= { register(`construction_inspection.draw.free_standing.h`) }
                        type= "number" sx= {{ minWidth: '70px', maxWidth: '50px' }} variant= "standard" 
                        InputProps= {{ disableUnderline: true, endAdornment: <InputAdornment position="end">mm</InputAdornment> }} />
                </Box>
            </Box>
            <Box sx= {{ margin: '0 20px', display: 'flex', flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                ...
                <Box sx= {{ border: 'solid 1px #dcdde1', borderRadius: '5px', padding: '5px 15px' }}>
                    <Ctrl.TextField name= { `construction_inspection.draw.free_standing.d` } register= { register(`construction_inspection.draw.free_standing.d`) }
                        type= "number" sx= {{ minWidth: '70px', maxWidth: '50px' }} variant= "standard" 
                        InputProps= {{ disableUnderline: true, endAdornment: <InputAdornment position="end">mm</InputAdornment> }} />
                </Box>
            </Box>
