                    ...
                    data.length !== 0 ? (
                        data.map((rows, index) => (
                            <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }} key= { index }>
                                ...
                                <StyledTableCell>{ rows.asset_name }</StyledTableCell>
                                <StyledTableCell>{ rows.user_fullname }</StyledTableCell>
                                <StyledTableCell align= "center">{ rows.quantity }</StyledTableCell>
                                ...
                                <StyledTableCell align= "center">
                                    <Box sx= {{ display: 'flex', flexDirectin: 'row', justifyContent: 'center', alignItems: 'center' }}>
                                        <Box sx= {{ margin: '5px' }}>
                                            <Link to= { `/issuance/assets/form/update/${rows.id}` } style= {{ textDecoration: 'none' }}>
                                        ...
                                        <Box sx= {{ margin: '5px' }}>
                                            <Link to= { `/issuance/assets/form/view/${rows.id}` } style= {{ textDecoration: 'none' }}>
                                                <Box padding= "8px 10px" color= "#ffffff" bgcolor= "#487eb0" 
                                                    borderRadius= "4px" display= "flex" flexDirection= "row" justifyContent= "center" alignItems= "center">
                        ...
                        <TableRow sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                            <StyledTableCell align= "center" colSpan= "7">No assigned assets found!</StyledTableCell>
                        </TableRow>
                    )
                ) : (
                    <TableRow>
