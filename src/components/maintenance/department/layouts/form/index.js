...
import { Department } from '../../../../../core/global/validation/Department';

// Constants
import { Department as Fields } from '../../../../../core/global/constants/Department';
...
const Index = () => {
    ...
    useEffect(() => {
        if(id !== undefined) get(id, 'department', setValue, setIsLoad);
    }, []);
    ...
    return (
        <Box sx= {{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch', marginBottom: '20px' }}>
            <Box sx= {{ padding: { xs: '0 14px', sm: 0 } }}>
                <Ctrl.Typography text= { `${type !== undefined ? type.charAt(0).toUpperCase() + type.slice(1) : ''} Department`} 
                ...
                <Grid container direction= "row" justifyContent= "flex-start" alignItems= "flex-start">
                    <Grid item xs= { 12 }>
                        ...
                        <Box width= "100%" marginTop= "30px" display= "flex" flexDirection= "row" justifyContent= "flex-end" aligItems= "center">
                            <Link to= "/maintenance/department" style= {{ textDecoration: 'none', width: '100px' }}>
                            ...
                            { type !== 'view' ? (
                                <Box marginLeft= "10px" width= "100px">
                                    <Ctrl.Button color= "primary" text= {
                                        ...
                                        ) } variant= "contained" onClick= { handleSubmit(data => save(id, data, type, 'department', '/maintenance/department', setError, setLoader, navigate) ) }/>
                                </Box>
                            ) : '' }
                        </Box>
                    </Grid>
