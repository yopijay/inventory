...
const Index = () => {
    ...
    useEffect(() => {
        excel(setXlsx, 'department', setIsLoad);
        getall(setDepartment, 'department', setIsLoad);
    }, []);
        ...
        <Box sx= {{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch', marginBottom: '20px' }}>
            <Box sx= {{ padding: { xs: '0 14px', sm: 0 } }}>
                <Grid container direction= "row"  justifyContent= "space-between" alignItems= "center">
                    <Grid item xs= { 12 } sm= { 2 }>
                        <Ctrl.Typography text= "Department List" 
                    ...
                    <Grid item xs= { 12 } sm= { 10 }>
                        <Grid container direction= "row" justifyContent= "flex-end" alignItems= "center">
                            <Grid item xs= { 4 } md= { 5 } lg= { 3 }>
                                <Link to= "/maintenance/department/form/new" style= {{ textDecoration: 'none' }}>
                                    <Ctrl.Button color= "primary" text= {
                                        <Ctrl.Typography color= "#ffffff" text= "New Department" 
                            ...
                            </Grid>
                            {
                                xlsx.length > 0 ? (
                                    <Grid item xs= { 1 }>
                                        <Export filename= "Department List" data= { xlsx } 
            ...
            <Box sx= {{ padding: { xs: '0 14px', sm: '0' } }}>
                <Box sx= {{ boxShadow: 2, borderRadius: '5px', overflow: 'hidden' }}>
                    <Paper elevation= { 0 }>
                        <TableContainer>
                            <Table aria-label= "Department" size= "small" sx= {{ minWidth: 650, maxHeight: 500 }} stickyHeader>
                                <Header />
                                <Body data= { department } isLoad= { isLoad } />
                            </Table>
                        </TableContainer>
                    </Paper>
                </Box>
