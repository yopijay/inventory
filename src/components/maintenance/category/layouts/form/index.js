    ...
    return (
        <Box sx= {{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch', marginBottom: '20px' }}>
            <Box sx= {{ padding: { xs: '0 14px', sm: 0 } }}>
                <Ctrl.Typography text= { `${type !== undefined ? type.charAt(0).toUpperCase() + type.slice(1) : ''} Category`} 
                            ...
                            { type !== 'view' ? (
                                <Box marginLeft= "10px" width= "100px">
                                    <Ctrl.Button color= "primary" text= {
                                        ...
                                        ) } variant= "contained" onClick= { handleSubmit(data => save(id, data, type, 'category', '/maintenance/category', setError, setLoader, navigate) ) }/>
                                </Box>
                            ) : '' }
                        </Box>
                    </Grid>
