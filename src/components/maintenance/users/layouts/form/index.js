    ...
    return (
        <Box sx= {{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'stretch', marginBottom: '20px' }}>
            <Box sx= {{ padding: { xs: '0 14px', sm: 0 } }}>
                <Ctrl.Typography text= { `${type !== undefined ? type.charAt(0).toUpperCase() + type.slice(1) : ''} User`} 
                ...
                <Grid container direction= "row" justifyContent= "flex-start" alignItems= "flex-start">
                    <Grid item xs= { 12 }>
                        ...
                        <Box width= "100%" marginTop= "30px" display= "flex" flexDirection= "row" justifyContent= "flex-end" aligItems= "center">
                            <Link to= "/maintenance/users" style= {{ textDecoration: 'none', width: '100px' }}>
                            ...
                            { type !== 'view' ? (
                                <Box marginLeft= "10px" width= "100px">
                                    <Ctrl.Button color= "primary" text= {
                                        ...
                                        ) } variant= "contained" onClick= { handleSubmit(data => save(id, data, type, 'users', '/maintenance/users', setError, setLoader, navigate) ) }/>
                                </Box>
                            ) : '' }
                        </Box>
                    </Grid>
